...
export async function getActiveOrder(tableId) {
  try {
    const response = await apiCall(
      `${API_BASE_URL}/orders?filters[table][id][$eq]=${tableId}&filters[status][$ne]=completed&filters[status][$ne]=cancelled&populate[order_items][populate][menu_item][populate][image]=*&sort=createdAt:desc&pagination[limit]=1`,
      "GET"
    );
...
export async function submitOrder(orderId, tableId) {
  try {
    ...
    const orderResponse = await updateOrderStatus(orderId, "accepted");
