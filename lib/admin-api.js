 ...
 */
export const getOrders = async (filters = {}, restaurantId = 5) => {
  let url = `${API_BASE_URL}/orders?populate=deep&sort=createdAt:desc&filters[restaurant][id][$eq]=${restaurantId}`;
  console.log("filters", filters);
  if (filters.status) {
    url += `&filters[status][$eq]=${filters.status}`;
  }
  if (filters.tableId) {
    url += `&filters[table][id][$eq]=${filters.tableId}`;
  }
 ...
 * Get all tables
 */
export const getTables = async (filters = {}, restaurantId = 5) => {
  let url = `${API_BASE_URL}/tables?populate=*&filters[restaurant][id][$eq]=${restaurantId}`;
 ...
 * Get all payments
 */
export const getPayments = async (filters = {}, restaurantId = 5) => {
  let url = `${API_BASE_URL}/payments?populate=deep&sort=payment_date:desc&filters[restaurant][id][$eq]=${restaurantId}`;
