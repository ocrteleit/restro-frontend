...
import { callWaiter, requestBill } from "@/lib/table-service";
...
async function callWaiterMutation(
  url: string,
  { arg }: { arg: { tableId: string } }
) {
  return callWaiter(Number.parseInt(arg.tableId));
}

async function requestBillMutation(
  url: string,
  { arg }: { arg: { tableId: string } }
) {
  return requestBill(Number.parseInt(arg.tableId));
}
...
export default function RestaurantMenuClient({
  ...
  const [animationType, setAnimationType] = useState<"waiter" | "bill" | null>(
    null
  );
  ...
  return (
    <div className="min-h-screen pb-24 bg-gradient-to-br from-orange-50 via-white to-pink-50">
      <header className="bg-gradient-to-r from-orange-500 via-pink-500 to-purple-600 text-white p-4 sm:p-6 text-center shadow-xl">
        <div className="relative">
          <div className="absolute inset-0 bg-black/10 rounded-3xl"></div>
          <div className="relative z-10 space-y-3 p-5">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold animate-fade-in bg-white/20 backdrop-blur-sm rounded-2xl py-3 px-6 inline-block">
              {restaurant?.name || "Loading..."}
            </h1>
            <div className="flex items-center justify-center gap-2 text-white/90 animate-fade-in animation-delay-100">
              <MapPin className="w-4 h-4" />
              <span className="text-sm sm:text-base">
                {restaurant?.location || "Restaurant Location"}
              </span>
            </div>
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/20 backdrop-blur-sm rounded-full text-sm font-medium animate-fade-in animation-delay-200">
      ...
      {/* Existing animation overlay code */}
      {(showWaiterAnimation || showBillAnimation) && (
        <div className="fixed inset-0 z-50 bg-gradient-to-br from-orange-500/20 via-purple-500/20 to-pink-500/20 backdrop-blur-md flex items-center justify-center">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl p-8 max-w-sm w-full mx-4 shadow-2xl border border-white/20">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-200/50 p-4 shadow-2xl">
